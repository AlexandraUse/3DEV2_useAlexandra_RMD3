...
function getClickCoordinatesHandler(coordinates){
    ...
    if(clickCount == 2){
        ...
        console.log("Get coordinates: " + "x: " + coordinates.x + " y: " + coordinates.y);
...
Polymer({
    ...
    onMovePlayer: function(player){
        ...
        selfCanvas.playerToMove = player;
    ...
    function(CanvasControl, CanvasInput, imgLoader, jsonLoader, TileField, pathfind) {
        ...
        function main(x, y, xrange, yrange, playerImages, bushesImages) {
            ...
            var context = CanvasControl.create(1000, 600, {
                //fillStyle: "url('css/img/bg_2.jpg')",
                //display: "block",
                marginLeft: "auto",
                marginRight: "auto"
            }, cnvs, 0, "cnvselement");
            ...
            input.mouse_move(function(coords) {
                mapLayers.map(function(layer) {
                    ...
                    //console.log("Move map coordinates: " + " x: " + tile_coordinates.x + " y: " + tile_coordinates.y);
                });
            });

            function newDraw(){
                if(selfCanvas.playerToMove == "undefined"){
                    ...
                    if(flag == false){
                        ...
                        if(calculatePaths === 100) {
                            flag = true;
                            switch(selfCanvas.playerToMove.player) {
                                case 1:
            ...
            function drawPlayer() {
                ...
                players.map(function(e) {
                    ...
                    pathfind(players[selfCanvas.playerToMove.player-1], [players[selfCanvas.playerToMove.player-1].xPos, players[selfCanvas.playerToMove.player-1].yPos], [selfCanvas.playerToMove.newX, selfCanvas.playerToMove.newY], mapLayers[0].getHeightLayout(), false, true).then(function (data) {
                        if (data.length > 0 && data[1] !== undefined) {
                            players[selfCanvas.playerToMove.player-1].xPos = data[1].x;
                            players[selfCanvas.playerToMove.player-1].yPos = data[1].y;
                        }
                    });
                });
                var playerPosNowX = players[selfCanvas.playerToMove.player-1].xPos;
                var playerPosNowY = players[selfCanvas.playerToMove.player-1].yPos;
                if(playerPosNowX == selfCanvas.playerToMove.newX && playerPosNowY == selfCanvas.playerToMove.newY){
                    for(var b in bushes){
